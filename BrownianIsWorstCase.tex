...
\begin{equation} \label{eqn:upper-recursion}
  \state_{\learnerM}(T)\odot \pot(T)=\upperscoreM(T)=\upperscoreM(T-1)=\cdots=\upperscoreM(0)=\upperpot(0,0)
\end{equation}
and
\begin{equation} \label{eqn:lower-recursion}
  \state_{\adversM}(T)\odot \pot(T)=\lowerscoreM(T)=\lowerscoreM(T-1)=\cdots=\lowerscoreM(0)=\lowerpot(0,0)
\end{equation}
...
\begin{theorem} \label{thm:IntegerGameBounds}
  ...
  \item
    ...
    $\state(i,\R_0)$. Guarantees a final potential 
     $$ \score(T) \geq \E{\R \sim \R_0 \oplus \Binom(T-i,1)}{\pot(T,\R)} $$
  \item
    There learner strategy~(\ref{eqn:learner-strat-1}) guarantees 
    $$\score(T) \leq \E{\R \sim \R_0 \oplus \Binom(T-i,2)}{\pot(T,\R)}$$
  \end{itemize}
\end{theorem}
...
Theorem~\ref{thm:IntegerGameBounds} do not match.
$\E{\R \sim \R_0 \oplus \Binom(T-i,1)}{\pot(T,\R)} < \E{\R \sim \R_0
  \oplus \Binom(T-i,2)}{\pot(T,\R)}$. In other words, the
...
descrete time game increases the options available to the adversary,
...
the adversary, indexed by $k$ and a single strategy for the learner. These strategies
are scaled versions of the strategies for the integer time
game~(\ref{eqn:adv-strat-p},\ref{eqn:learner-strat-1}).

Adversarial strategy $\adversMdk$ is a scaled version of the integer
strategy (Eqn~(\ref{eqn:adv-strat-p})):
\begin{equation} \label{eqn:adv-strat-dk}
  \adversMdk=  \begin{cases}
    +2^{-k} & \mbox{ w.p. } \frac{1}{2}\\
    -2^{-k} & \mbox{ w.p. } \frac{1}{2}\\
  \end{cases}
\end{equation}

The learner chooses the distribution over action after the adversary
chooses the step size, which is $s_k$ on all iterations. Recall that $t_{i+1}=t_i+s_k^2$.
\begin{eqnarray} \label{eqn:learner-strat-1c}
  \learnerMd(t_{i},\R) = \frac{1}{Z^{1d}}
  \frac{\pot(t_{i+1},\R+s_k(1+s_k)) -
  \pot(t_{i+1},\R-s_k(1+s_k))}{2} \\
  \mbox{ where } Z^{1d} = \E{\R \sim \state(t_{i+1})}{\frac{\pot(t_{i+1},\R+s_k(1+s_k)) -
  \pot(t_{i+1},\R-s_k(1+s_k)}{2}} \nonumber
\end{eqnarray}
The learner strategy $\learnerMd$ is identical to $\learnerMb$
(Eqn~(\ref{eqn:learner-strat-1})) when $s_k=1$, but is different when
$s_k<1$. In particular when $s_k \to 0$, $s_k(1+s_k) \to s_k$, which,
as we show below, reduces the gap between the upper and lower
potentials to zero.

\begin{theorem} \label{thm:IntegerGameBounds}
  Let $\pot_T \in \SP{2}$, fix the step size $s_k=2^{-k}$, let
  $\realT$ be an integer multiple of $s_k$ define
  $T=\frac{\realT}{s_k^2}$ and let $t_i=i s_k^2$.
  For any iteration $0 \leq i < T$ and
  initial regret $\R_0 \in \reals$ we define $\state(t_i,\R_0)$ to
  contain all paths that are equal to $\R_0$ on iteration $i$. We
  consider the final score $\score(\realT)$ starting from state
  $\state(t_i,\R_0)$ and using a particular strategy
  \begin{itemize}
  \item
    The adversarial strategy~(Eqn~(\ref{eqn:adv-strat-dk})) starting from
    $\state(t_i,\R_0)$. Guarantees a final potential 
     $$ \score(\realT) \geq \E{\R \sim \R_0 \oplus \Binom(T-i,s_k)}{\pot(\realT,\R)} $$
  \item
    There learner strategy~(\ref{eqn:learner-strat-1c}) guarantees 
    $$\score(\realT) \leq \E{\R \sim \R_0 \oplus \Binom(T-i,s_k(1+s_k))}{\pot(\realT,\R)}$$
  \end{itemize}
\end{theorem}
