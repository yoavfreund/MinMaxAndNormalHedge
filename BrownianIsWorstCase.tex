...
game that are a min/max pair, but we conjecture that is not the case.

In order to close the gap and find min/max strategies we extend the
game by giving the adversary additional choices,
we call the extended game the {\em discrete time game}. While
in the integer time game the loss of each action is in the range $[-1,+1]$,
in the discrete time game the adversary chooses, on iteration $i$ a
step size $0<s_i\leq 1$ which restricts the losses to be in the range
$[-s_i,+s_i]$. Note that by always choosing $s_i=1$, the adversary can
recreate the integer time game.

We give an informal argument for using the total variance instead of
the number of steps. Suppose we use the adversarial
strategy~(\label{eqn:adv-strat-p}) for $N$ iterations and that $N$ is
large. The central limit theorem implies that $B(N,1) \approx {\cal N}(0,N)$
i.e. the normal distribution with mean 0 and variance $N$. therefor
the final potential is approximately 
$$\score(N) \approx \E{\R \sim {\cal N}(0,N)}{\pot(N,\R)}$$

Suppose we half the step size $s_i=1/2$ for all $0\leq i \leq N-1$. 
The variance in each step decreases to $1/4$, so the total variance is
$N/4$. and the final score is
$$\score(N) \approx \E{\R \sim {\cal N}(0,\frac{N}{4})}{\pot(N,\R)}$$
To keep the total variance unchanged we need to {\em quadruple} the
number of iterations from $N$ to $4N$. In general, we need to replace
the number of iterations $N$ with the total variance. Said
differently, we replace the iteration counter $i$ with 
a real values notion of time, which is the cumulative variance: $t_i =
\sum_{j=1}^i s^i$. As expected, in the integer time game $s_i=1$ which
implies that $t_i = i$


Without further restrictions, this game allows the cumulative loss of
the algorithm at a fixed time $t_N=1$ to grow infinitely large.
For example, suppose the conditional distribution of losses at each
iteration $i$ and regret value $\R$
\begin{equation} \label{eqn:adv-strat-k}
  \adversM^k(i,\R) =
  \begin{cases}
    +1/k & \mbox{ w.p. } \frac{3}{4}\\
    -1/k & \mbox{ w.p. } \frac{1}{4}\\
  \end{cases}
\end{equation}
...
$s_i^2$.~\footnote{We conjecture that removing this requirements
