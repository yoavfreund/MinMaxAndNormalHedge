...
a random variable which maps each action $\omega \in \Omega$
to the cumulative loss of that action: $L_\omega^i$.
...
In the the potential game, only the final potential function is
known. However, as we will now show, there is a natural way to define
a potential function for all iterations.

Fix the strategies for both sides $\learnerM(\cdot,\cdot), \adversM(\cdot,\cdot)$. We
define the fixed strategies potential $\potPQ(i,R)$ using backward
induction. The base case is  $\potPQ(T,\R) = \pot(T,\R)$.

We assume that  $\potPQ(i+1,\R)$ is known and compute  $\potPQ(i,\R)$
We use our knowledge of $\learnerM(i,\R)$ and
Equations~(\ref{eqn:Bias},\ref{eqn:aggregate-loss}) to calculate
$\ell(i)$. Define the potential as:
...
\begin{equation} \label{eqn:Vn}
  ...
  \sum_{i=1}^n \E{\R \sim \state(t_i)}{\E{y \sim \adversM(t_i,R)}{\adversM(t_i,\R)
      (y-\ell(t_i))^2}} 
\end{equation}
