...
\begin{lemma} \label{lemma:first-order-bound}
  ...
  \begin{enumerate}
    ...
    guarantees the lower potential~(\ref{eqn:backward-iteration-lower})
    \item The learner strategy~(\ref{eqn:learner-strat-1})
      guarantees the upper potential~(\ref{eqn:backward-iteration-upper-recursion})
    \end{enumerate}
\end{lemma}
...
backwards for $i=T,T-1,T-2,\ldots$
...
identical when $s=1$. As we show below, when $s_k \to 0$ both the
upper and lower potential converge to the same limit.
...
\begin{theorem} \label{thm:IntegerGameBounds}
  ...
  \item
    ...
    $\state(t_i,\R_0)$. Guarantees a final potential 
     $$ \score(\realT) \geq \E{\R \sim \R_0 \oplus \Binom(T-i,s_k)}{\finalPotT(\R)} $$
  \item
    There learner strategy~(\ref{eqn:learner-strat-1c}) guarantees 
    $$\score(\realT) \leq \E{\R \sim \R_0 \oplus \Binom(T-i,s_k(1+s_k))}{\finalPotT(\R)}$$
  \end{itemize}
\end{theorem}
