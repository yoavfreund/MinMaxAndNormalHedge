...
\begin{equation} \label{eqn:learner-strat-cc}
  \learnerM^{NH2}(V_i,\R) =  \frac{1}{Z} Re^{\frac{R^2}{2V_i}}
  \mbox{ where } Z = \E{\R \sim \state_i}{Re^{\frac{R^2}{2V_i}}}
\end{equation}

\item The aggregate loss is calculated:
  \begin{equation} %\label{eqn:ell-discrete}
    \ell_i=\E{\R \sim \state(i)}{\learnerM^{NH2}(V_i,\R)
      \Bias(V_i,\R)},\;\mbox{ where } \Bias(V_i,\R) \doteq \E{y \sim \adversM(V_i,\R)}{y}
  \end{equation}
  
\item  Increment $V_{i+1}=V_i +Var_i$ where
\begin{equation} \label{eqn:deltat}
  Var_i=
  \E{\R \sim \state(i)}{H(t_i,\R) \;\; \E{y \sim \adversM(i,\R)}{(y-\ell_i)^2}}
\end{equation}
\begin{eqnarray}
    \mbox{ where } W(t_i,\R)&=& e^{\frac{(R+1)^2}{2t_i}} \left(\frac{1}{t^{3/2}} + \frac{(R+1)^2}{t^{5/2}} \right)\\
   H(t_i,\R)&=& \frac{W(t_i,R)}{\E{\rho \sim \state(i)}{W(t_i,\rho)}}
\end{eqnarray}

\item The state is updated.
  $$\state(i+1) = \E{\R \sim \state(i)}{\adversM(i,\R)\oplus (\R-\ell_i)}
  $$
\end{enumerate}
\end{minipage}}
