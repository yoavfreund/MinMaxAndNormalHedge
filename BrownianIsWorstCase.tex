...
\begin{definition}[Simultaneous regret
  ...
  A distribution over the reals $\state$ is satisfies simultaneous regret bound by $G$ if
  \[
...
\begin{theorem}\label{thm:simulBoundAveragePot}
  ~\\
  \begin{itemize}
  \item $\state$ satisfies SRB for $G$ if it satisfies APB for $\pot(R) = G(R)^{-1}$.
  \item A distribution $\state$ satisfies an APB for $\pot$ if it
    satisfies SRB for $G$ and
    $\int_{-\infty}^{\infty}\pot(\R) G(\R) d\R \leq 1$
   \end{itemize}
\end{theorem}
...
\begin{eqnarray}
  ...
  H(t_i,\R)&=& \frac{W(t_i,R)}{\E{\rho \sim \state(t_i)}{W(t_i,\rho)}}
\end{eqnarray}
