...
\begin{definition}[Simultaneous regret
...
$\pot:\reals \to \reals$. Potential based learning algorithm are
...
simultaneous bounds and average potential bounds. 
...
Simultaneous regret bounds are more intuitive than potential
...
It is not hard to show that the lattices get finer with $k$, i.e. if  $k\geq j$,  $\Klat{\realT,j} \subseteq \Klat{\realT,k}$.
...
\begin{theorem}\label{thm:smallerSteps}
  If $ \finalPotR \in \SP4$, and $\realT>0$ then 
  then for any integer $k>0$
  $$\lowerpotMdkpar{k+1} >  \lowerpotMdkpar{k}$$
\end{theorem}

The proof of the theorem relies on reduction to a simpler case: dividing a single time step into two equal parts.

\begin{lemma} \label{lemma:half-step}
  If $ \finalPotR \in \SP4$, and $\realT>0$ then 
  $$\lowerpotMdkpar{1} >  \lowerpotMdkpar{0}$$
\end{lemma}
  
\proof The step size $s_k=2^{-k}\sqrt{\realT}$, therefor
$s_0=\sqrt{\realT}, s_1=\frac{\realT}{2}$. The time step
$\Delta t_k = s_k^2$, therefor
$\Delta t_0=\realT, \Delta t_1=\frac{\realT}{4}$. In other words,
$k=0$ corresponds to a single step of size , while $k=1$ corresponds
to four steps of size $\frac{1}{2}$.

By definition $\forall k,\; \finalPotR(R)=\lowerpotMdkpar{k}(\realT,\R)$

For $k=0$ we we get the recursion
\begin{equation}
  \lowerpotMdkpar{0}(0, \R) =
  \frac{\lowerpotMdkpar{0}(\realT,\R-\sqrt{\realT})+
    \lowerpotMdkpar{0}(\realT,\R+\sqrt{\realT})}{2}
  =   \frac{\finalPotR(\R-\sqrt{\realT})+
    \finalPotR(\R+\sqrt{\realT})}{2}
\end{equation}

For $k=1$ we we have for $i=0,1,2,3$:
\begin{equation}   \label{eqn:lowerpotquarterstep}
   \lowerpotMdkpar{0}\paren{\frac{i}{4}\realT,\R}=
 \frac{\lowerpotMdkpar{0}\paren{\frac{i+1}{4}\realT,\R-\frac{1}{2}\sqrt{\realT}}+
   \lowerpotMdkpar{0}\paren{\frac{i+1}{4}\realT,\R+\frac{1}{2}\sqrt{\realT}}}{2}
 \end{equation}

\newcommand{\iter}[1]{\lowerpotMdkpar{1}\paren{\realT,\R {#1} \sqrt{\realT}}}
\newcommand{\iterzero}{\lowerpotMdkpar{1}\paren{\realT,\R}}

Combining Equation~(\ref{eqn:lowerpotquarterstep}) fr $k=0,1,2,3$ we get

\small
\begin{eqnarray} 
  \lowerpotMdkpar{1}(0, \R) = 
  \frac{1}{16}
  &&\left[\iter{-2}+4\iter{-}+6\iterzero \right.\\
        && + \left. 4\iter{+}+\iter{+2}\right]
\end{eqnarray}
\normalsize
...
% \paren{\pot(t',\R+2s)+ 4\pot(t',\R+s)+ 6\pot(t',\R)+  4\pot(t',\R-s)+ \pot(t',\R-2s)}
 ...
 $$ \pot_{k+1}(t, \R)-\pot_{k}(t, \R) = \frac{1}{16}
  ...
  Our goal is to show that the RHS is positive. Therefor we can
